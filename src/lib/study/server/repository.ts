...
export class StudyRepository {
  ...
  auditFor(target: string) {
    return this.db.prepare('SELECT id, at, action, detail FROM audit WHERE target=? ORDER BY id').all(target).map(row => ({ ...row, detail: JSON.parse(String(row.detail)) as unknown }));
  }
  ...
  audit(action: string, target: string, detail: unknown = {}) {
    this.db
      .prepare('INSERT INTO audit(at,action,target,detail) VALUES(?,?,?,?)')
      .run(new Date().toISOString(), action, target, JSON.stringify(detail));
  }
  ...
  read(relative: string) {
    if (relative.includes('..') || path.isAbsolute(relative))
      throw new Error('文件标识无效');
    return readFileSync(path.join(this.root, relative));
  }
  remove(relative: string) {
    if (relative.includes('..') || path.isAbsolute(relative))
      throw new Error('文件标识无效');
    rmSync(path.join(this.root, relative), { recursive: true, force: true });
  }
