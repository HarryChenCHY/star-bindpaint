...
const tools: Record<Topic, [LucideIcon, string, string][]> = {
  ...
  follow: [[Wand2, '星光画笔', '调节笔宽，数字显示四舍五入后的大小；你主动选定的大小会保留。'], [Layers, '笔迹方式', '“AI 修正”把通过判定的手绘轨迹替换成规划笔触；“保留原笔”保留你亲手画出的形状。'], [Music2, '节奏', '“自己画”由你逐笔绘画；“一起画”让你完成一笔后，伙伴继续补若干笔，比例可调。'], [Sparkles, '月亮伙伴帮画', '可以补 5、10、20 笔或全部画完。伙伴补笔单独记录，不等同于亲手绘画。'], [ChevronsRight, '换一笔', '跳过当前引导，寻找下一笔；不是完成当前笔，也不会把跳过的笔自动涂上。']],
  auto: [[Play, '自动续画 / 暂停', '播放时图标变为暂停，再点立即暂停并返回跟画；再次播放从当前进度继续。'], [ChevronsRight, '快慢', '设置相邻笔触的播放间隔，数值越小越快。0 为最快。'], [Wand2, '返回沿星迹', '停止自动续画，由你接着当前星迹画。'], [Sparkles, '切换自由星域', '停止续画后进入自由绘画，已完成的笔触保留。']],
  ...
  companion: [[Lock, '锁定 / 解锁', '默认锁定。解锁后拖动顶部点状手柄移动提示卡，也可聚焦手柄用方向键微调。再次锁定防止误拖。'], [Minus, '收起 / 展开', '减号把提示卡缩成方圆月亮图标，点图标展开。位置和锁定状态会被记住。'], [Maximize2, '参考原图', '有参考图片时显示在卡片顶部。点击全屏放大，再点击图片、关闭或按 Esc 返回。'], [Sparkles, '完成剩余笔触', '让月亮伙伴从当前位置自动完成余下笔触；播放时这里变为暂停按钮。'], [Layers, '完整 / 适度 / 起点', '完整：30% 紫色填充、虚线外框、1/2 标记和箭头。适度：起终点与方向线。起点：只显示圆形 1。短到两点重叠的笔触合并提示，轻点或短划即可。']],
