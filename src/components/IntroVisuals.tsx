...
const STYLE_NOTES = [
  ...
  ['鼓起厚涂', '笔宽在中段增大，双层描画形成厚涂感，色相与饱和度变化更明显。', '适合有力度的线条和密集排笔；旋转方向由你的手势决定。'],
  ['鲜艳平涂', '接近等宽的笔刷、较小的边缘扰动与较高不透明度，提升色彩饱和度。', '适合铺设大块颜色和清晰的色彩分区。'],
  ['压感干笔', '宽度随压力变化，降低饱和度，渲染时随机略过约 15% 的线段。', '适合有留白的肌理。鼠标采用基础压力，压感笔可增加提按变化。'],
  ['断续笔迹', '近似等宽、分段处理与较强色相变化；周期性留出笔迹间隙。', '适合碎片化线条和色彩实验；不会自动重构物体的几何形状。'],
  ['轻盈渐细', '两端收细、低边缘扰动和较低不透明度，形成轻薄的重叠。', '适合流畅勾画与轻柔叠色；不是物理水彩扩散模拟。'],
];
...
export function StyleExplorer() {
  ...
  return <>
    ...
    <p className="intro-caption">六条示例使用同一条输入路径、同一基础颜色和笔宽，由自由画板的实际渲染函数绘制。随机肌理每次可能略有不同；这是本地程序化笔刷，不需要调用大模型。</p>
  </>;
}
