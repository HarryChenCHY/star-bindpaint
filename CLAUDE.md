...
- 当前新增 `/study` 全流程、`/admin/studies` 研究后台、`/rater` 双人评分、`/simplify` 200 笔以内简化画体验。详细状态与使用说明见 `论文/程序改造_实施记录与使用说明.md`。
- 本轮研究聚焦同一参与者、同一简化图的 AB/BA 单次任务比较；长期练习不是本次主要实验终点。
- 研究共享画布为 `src/components/study/StudyCanvas.tsx`，领域逻辑在 `src/lib/study/`，区域预算规划在 `src/lib/stroke-planner/`；原有油画体验保持独立。
